...
- Conserve le secteur en termes génériques, la taille approximative, le contexte utile, le besoin, les contraintes, le calendrier et le budget s'ils ont été exprimés.
  ...
  try {
    out = await askClaudeJSON({
      ...
      messages: [
        {
          role: "user",
          content: `Synthèse interne : ${need.need_summary ?? "(aucune)"}\n\nConversation :\n${transcript}`,
        },
      ],
    });
  } catch (e: any) {
